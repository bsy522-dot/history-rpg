// ============================================================
// battle.js — 한국사 영웅전 v9 Tactical Battle System
// ============================================================
...
function drawUnitSprite(sx,sy,unit,isSelected,frameIdx){
  ...
  if(cls.includes('궁')||cls==='명궁'||cls==='신궁'||cls==='한군궁수'){
    ...
    X.fillStyle='#8a6a2a';X.fillRect(8,-6,6,3);
    ...
    X.fillStyle='#6aaacc';X.fillRect(-7,-14,14,5);
  }else if(cls==='우사'||cls==='대우사'){
    X.fillStyle='#4a6aaa';X.fillRect(-7,-14,14,5);
  }else if(cls==='운사'||cls==='대운사'){
    X.fillStyle='#7a7aaa';X.fillRect(-7,-14,14,5);
  }
  if(isBoss){
...
function loadTacWave(idx,tacData){
  ...
  const diffMul=DIFFICULTY[G.difficulty]||DIFFICULTY[1];
  tc.enemies=wave.map((e,i)=>{
    const pos=positions[i]||[3+i,0];
    const en={...e,alive:true,acted:false,moved:false,stunned:false,buffs:[],ally:false,
      gx:pos[0],gy:pos[1],_sx:0,_sy:0,enraged:false,
      mov:e.mov||(CLS[e.cl]||{}).mov||4,rng:e.rng||(CLS[e.cl]||{}).rng||1};
    // Apply difficulty multipliers
    en.mhp=Math.floor((en.mhp||en.hp||100)*diffMul.hpMul);
    en.hp=en.mhp;
    en.atk=Math.floor((en.atk||10)*diffMul.atkMul);
    return en;
  });
}
...
function calcMoveRange(unit){
  ...
  while(q.length){
    ...
    [[0,1],[0,-1],[1,0],[-1,0]].forEach(([dx,dy])=>{
      ...
      if(occByEnemy)return;
      if(visited.has(k)&&visited.get(k)>=nrem)return;
      visited.set(k,nrem);
      if(!occByAlly)range.push({x:nx,y:ny});
      if(!isZoc&&!occByAlly)q.push([nx,ny,nrem]);
      else if(isZoc&&!occByAlly){}
    });
  }
  const seen=new Set();
...
function rTacBattle(){
  ...
  info+=` | ${DIFFICULTY[G.difficulty].name}`;
...
// ===== v9.0: PARTICLE EFFECTS =====

function spawnParticles(x,y,color,count){
  const canvas=document.getElementById('particle-fx');
  if(!canvas)return;
...
function execAttack(a,t){
  ...
  const spd=G.battleSpeed||1;
  if(miss){sfx('miss');fDmg(t,0,'miss');setTacInfo(`${a.nm}의 공격! ${t.nm} 회피!`);addBattleLog(`${a.nm}→${t.nm} 회피!`)}
  else{sfx(crit?'crit':'hit');shake(crit?300:150);t.hp=Math.max(0,t.hp-dmg);fDmg(t,dmg,'phys',crit);
    let m=`${a.nm}→${t.nm} ${dmg}피해!`;if(crit)m+=' [크리티컬!]';if(ta>1)m+=' [유리!]';if(ta<1)m+=' [불리]';setTacInfo(m);addBattleLog(m);
    if(t.hp<=0){t.alive=false;t.hp=0}
    chkEnrage(t);
    spawnParticles(t._sx||C.width/2,t._sy||C.height/2,'#ff6644',8);
  }
  // Counterattack
  if(t.alive && !t._countered && t.rng >= (Math.abs(a.gx-t.gx)+Math.abs(a.gy-t.gy))){
    ...
    setTimeout(()=>{
      ...
      if(cr.miss){sfx('miss');fDmg(a,0,'miss');setTacInfo(`${t.nm}의 반격! ${a.nm} 회피!`);addBattleLog(`${t.nm}→${a.nm} 반격 회피!`)}
      else{sfx('hit');shake(80);a.hp=Math.max(0,a.hp-cr.dmg);fDmg(a,cr.dmg,'phys',cr.crit);
        setTacInfo(`${t.nm}의 반격! ${a.nm}에게 ${cr.dmg}피해!`);addBattleLog(`${t.nm} 반격! ${a.nm}에게 ${cr.dmg}피해`);
        if(a.hp<=0){a.alive=false;a.hp=0}}
    },Math.floor(400/spd));
  }
  a.acted=true;G.tac.atkRange=[];G.tac.moveRange=[];G.tac.selUnit=null;
  clearTacActions();
  setTimeout(()=>{if(!checkBattleEnd())checkAllyPhaseEnd()},Math.floor(600/spd));
}

function execSkill(a,t,sk){
  a.mp-=sk.mp;sfx('skill');
  const spd=G.battleSpeed||1;
  // Particle colors by skill type
  const skColor=sk.t.includes('heal')?'#44ff66':sk.n.includes('화')||sk.n.includes('불')?'#ff4422':sk.n.includes('뇌')||sk.n.includes('번개')?'#ffdd00':sk.n.includes('얼음')||sk.n.includes('빙')?'#44aaff':'#aa66ff';
  if(sk.t==='single'||sk.t==='charge'){
    const{dmg,crit,miss}=calcDmg(a,t,sk.pw);
    if(miss){fDmg(t,0,'miss');setTacInfo(`${a.nm}의 ${sk.n}! ${t.nm} 회피!`);addBattleLog(`${a.nm} ${sk.n}→${t.nm} 회피!`)}
      ...
      setTacInfo(m);addBattleLog(m);if(t.hp<=0){t.alive=false;t.hp=0}chkEnrage(t);
      spawnParticles(t._sx||C.width/2,t._sy||C.height/2,skColor,15);
    }
  }else if(sk.t==='multi'){
    let total=0;
    for(let h=0;h<(sk.h||2);h++){
      setTimeout(()=>{if(!t.alive)return;const{dmg}=calcDmg(a,t,sk.pw);t.hp=Math.max(0,t.hp-dmg);fDmg(t,dmg,'phys');sfx('hit');shake(100);total+=dmg;if(t.hp<=0){t.alive=false;t.hp=0}},Math.floor(h*200/spd));
    }
    setTacInfo(`${a.nm}의 ${sk.n}! ${sk.h||2}연타!`);addBattleLog(`${a.nm} ${sk.n} ${sk.h||2}연타!`);
    spawnParticles(t._sx||C.width/2,t._sy||C.height/2,skColor,12);
  }else if(sk.t==='drain'){
    ...
    setTacInfo(`${a.nm}의 ${sk.n}! ${dmg}피해+${hl}흡수!`);addBattleLog(`${a.nm} ${sk.n} ${dmg}피해+${hl}흡수`);if(t.hp<=0){t.alive=false;t.hp=0}
    ...
    targets.forEach((et,i)=>setTimeout(()=>{
      ...
      spawnParticles(et._sx||C.width/2,et._sy||C.height/2,skColor,10);
    },Math.floor(i*100/spd)));
    setTacInfo(`${a.nm}의 ${sk.n}! 범위 공격!`);addBattleLog(`${a.nm} ${sk.n} 범위 공격! ${targets.length}명 피격`);
    ...
    targets.forEach((al,i)=>setTimeout(()=>{
      ...
      spawnParticles(al._sx||C.width/2,al._sy||C.height/2,'#44ff66',8);
    },Math.floor(i*100/spd)));
    setTacInfo(`${a.nm}의 ${sk.n}! 범위 회복!`);addBattleLog(`${a.nm} ${sk.n} 범위 회복`);
  }else if(sk.t==='revive'){
    if(!t.alive){t.alive=true;t.hp=Math.floor(t.mhp*.5);fDmg(t,t.hp,'heal');sfx('heal');setTacInfo(`${a.nm}의 ${sk.n}! ${t.nm} 부활!`);addBattleLog(`${a.nm} ${sk.n}! ${t.nm} 부활!`);
      spawnParticles(t._sx||C.width/2,t._sy||C.height/2,'#44ff66',20);
    }
  ...
  setTimeout(()=>{if(!checkBattleEnd())checkAllyPhaseEnd()},Math.floor(700/spd));
}

function execSelfSkill(u,sk){
  u.mp-=sk.mp;sfx('skill');
  if(sk.t==='buff_def'){u.buffs.push({t:'def',turns:3,v:sk.pw});setTacInfo(`${u.nm}의 ${sk.n}! 방어력 증가!`);addBattleLog(`${u.nm} ${sk.n} 방어력 증가`)}
  else if(sk.t==='buff_eva'){u.buffs.push({t:'eva',turns:3,v:sk.pw});setTacInfo(`${u.nm}의 ${sk.n}! 회피 증가!`);addBattleLog(`${u.nm} ${sk.n} 회피 증가`)}
  else if(sk.t==='buff_mov'){u.buffs.push({t:'mov',turns:3,v:sk.pw});setTacInfo(`${u.nm}의 ${sk.n}! 이동력 증가!`);addBattleLog(`${u.nm} ${sk.n} 이동력 증가`)}
  u.acted=true;G.tac.selUnit=null;clearTacActions();
  setTimeout(()=>checkAllyPhaseEnd(),Math.floor(500/(G.battleSpeed||1)));
}

function execBuffSkill(a,sk){
  const rad=sk.rad||2;const tc=G.tac;
  const targets=tc.allies.filter(al=>al.alive&&Math.abs(al.gx-a.gx)+Math.abs(al.gy-a.gy)<=rad);
  if(sk.t==='buff_atk_area'){targets.forEach(al=>al.buffs.push({t:'atk',turns:3,v:sk.pw}));setTacInfo(`${a.nm}의 ${sk.n}! 범위 공격력 증가!`);addBattleLog(`${a.nm} ${sk.n} 범위 공격력 증가`)}
  else if(sk.t==='buff_def_area'){targets.forEach(al=>al.buffs.push({t:'def',turns:3,v:sk.pw}));setTacInfo(`${a.nm}의 ${sk.n}! 범위 방어력 증가!`);addBattleLog(`${a.nm} ${sk.n} 범위 방어력 증가`)}
  else if(sk.t==='king_buff_area'){targets.forEach(al=>{al.buffs.push({t:'atk',turns:3,v:1.3});al.buffs.push({t:'def',turns:3,v:1.3})});setTacInfo(`${a.nm}의 ${sk.n}! 범위 능력 강화!`);addBattleLog(`${a.nm} ${sk.n} 범위 능력 강화`)}
  sfx('skill');
}
...
function execDebuffSkill(a,t,sk){
  ...
  setTacInfo(`${a.nm}의 ${sk.n}! 적 명중 감소!`);addBattleLog(`${a.nm} ${sk.n} 적 명중 감소`);sfx('skill');
}

function execItemUse(id,t){
  const it=ITEMS[id],inv=G.inv.find(i=>i.id===id);if(!inv||inv.q<=0)return;inv.q--;
  if(it.heal&&t.alive){t.hp=Math.min(t.mhp,t.hp+it.heal);fDmg(t,it.heal,'heal');sfx('heal');setTacInfo(`${it.n} 사용! HP ${it.heal} 회복!`);addBattleLog(`${it.n} 사용! ${t.nm} HP ${it.heal} 회복`)}
  if(it.hmp&&t.alive){t.mp=Math.min(t.mmp,t.mp+(it.hmp||0));if(it.hmp)fDmg(t,it.hmp,'heal')}
  if(it.rev&&!t.alive){t.alive=true;t.hp=Math.floor(t.mhp*it.rpct);fDmg(t,t.hp,'heal');sfx('heal');setTacInfo(`${t.nm} 부활!`);addBattleLog(`${t.nm} 부활!`)}
  const u=G.tac.selUnit;if(u)u.acted=true;
  G.tac.skillRange=[];G.tac.selUnit=null;clearTacActions();
  setTimeout(()=>checkAllyPhaseEnd(),Math.floor(500/(G.battleSpeed||1)));
}

function chkEnrage(t){if(t.isBoss&&t.enrageAt&&t.alive&&t.hp/t.mhp<=t.enrageAt&&!t.enraged){t.enraged=true;t.atk=Math.floor(t.atk*1.5);t.spd+=3;setTacInfo(`${t.nm} 분노! 공격력 대폭 증가!`);addBattleLog(`${t.nm} 분노! 공격력 대폭 증가!`)}}
...
function checkAllyPhaseEnd(){
  ...
  if(tc.allies.every(u=>!u.alive||u.acted)){
    setTimeout(startEnemyPhase,Math.floor(500/(G.battleSpeed||1)));
  }else{
...
function startEnemyPhase(){
  ...
  clearTacActions();sfx('phase');addBattleLog('--- 적군 페이즈 ---');
  ...
  setTimeout(()=>processEnemyAI(0),Math.floor(600/(G.battleSpeed||1)));
}
...
function processEnemyAI(idx){
  ...
  if(idx>=activeEnemies.length){
    setTimeout(startNewTurn,Math.floor(500/(G.battleSpeed||1)));return;
  }
  const en=activeEnemies[idx];
  enemyAI(en,()=>setTimeout(()=>processEnemyAI(idx+1),Math.floor(400/(G.battleSpeed||1))));
}
    ...
    if(en.skills?.length&&Math.random()<skillChance&&en.mp>=(en.skills[0].mp||0)){
      ...
      if(en.mp>=(sk.mp||0)){
        ...
        if(sk.t==='aoe_area'){
          ...
          hits.forEach((t,i)=>setTimeout(()=>{const{dmg}=calcDmg(en,t,sk.pw);t.hp=Math.max(0,t.hp-dmg);fDmg(t,dmg,'magic');if(t.hp<=0){t.alive=false;t.hp=0}},Math.floor(i*100/spd)));
          setTacInfo(`${en.nm}의 ${sk.n}!`);addBattleLog(`${en.nm} ${sk.n} 범위 공격!`);
        }else{
          ...
          setTacInfo(`${en.nm}의 ${sk.n}!`);addBattleLog(`${en.nm} ${sk.n}→${bestTarget.nm}`);
        }
        en.acted=true;
        setTimeout(()=>{checkBattleEnd();cb()},Math.floor(500/spd));return;
      }
    }
    const{dmg,crit,miss}=calcDmg(en,bestTarget);
    if(miss){sfx('miss');fDmg(bestTarget,0,'miss');setTacInfo(`${en.nm}→${bestTarget.nm} 회피!`);addBattleLog(`${en.nm}→${bestTarget.nm} 회피!`)}
    else{sfx(crit?'crit':'hit');shake(crit?250:120);bestTarget.hp=Math.max(0,bestTarget.hp-dmg);fDmg(bestTarget,dmg,'phys',crit);
      setTacInfo(`${en.nm}→${bestTarget.nm} ${dmg}피해!`);addBattleLog(`${en.nm}→${bestTarget.nm} ${dmg}피해`);if(bestTarget.hp<=0){bestTarget.alive=false;bestTarget.hp=0}}
    en.acted=true;
    setTimeout(()=>{checkBattleEnd();cb()},Math.floor(500/spd));
  }else{
    ...
    if(distAfter<=en.rng){
      setTimeout(()=>{
        ...
        if(en.isBoss&&en.skills?.length&&Math.random()<0.4&&en.mp>=(en.skills[0].mp||0)){
          ...
          if(en.mp>=(sk.mp||0)){
            ...
            if(sk.t==='aoe_area'){
              ...
              setTacInfo(`${en.nm}의 ${sk.n}!`);addBattleLog(`${en.nm} ${sk.n}→${atkTarget.nm}`);
            }
            en.acted=true;
            setTimeout(()=>{checkBattleEnd();cb()},Math.floor(400/spd));return;
          }
        }
        ...
        if(miss){sfx('miss');fDmg(atkTarget,0,'miss')}
          ...
          if(atkTarget.hp<=0){atkTarget.alive=false;atkTarget.hp=0}}
        setTacInfo(`${en.nm}→${atkTarget.nm} ${miss?'회피!':dmg+'피해!'}`);addBattleLog(`${en.nm}→${atkTarget.nm} ${miss?'회피':dmg+'피해'}`);
        en.acted=true;setTimeout(()=>{checkBattleEnd();cb()},Math.floor(400/spd));
      },Math.floor(300/spd));
    }else{
      en.acted=true;cb();
    }
    ...
    stData.reinforcements.forEach(r=>{
      if(r.turn===tc.turn){
        ...
        r.enemies.forEach((e,i)=>{
          ...
          const en={...e,alive:true,acted:false,moved:false,stunned:false,
        ...
        setTacInfo('⚠️ 적 증원 등장!');sfx('phase');addBattleLog('적 증원 등장!');
      }
    });
  }
...
function checkBattleEnd(){
  ...
  if(!ae.length){
    if(tc.waves&&tc.waveIdx<tc.waves.length-1){
      setTacInfo('Wave 클리어!');addBattleLog('Wave 클리어!');
      setTimeout(()=>{
        const tacData=STAGES[G.stage].tacBattle;
        ...
        $('tac-phase').textContent='아군 페이즈';
      },Math.floor(1000/(G.battleSpeed||1)));return true;
    }
    setTimeout(showVic,Math.floor(500/(G.battleSpeed||1)));return true;
  }
...
function showVic(){
  ...
  const diffMul=DIFFICULTY[G.difficulty]||DIFFICULTY[1];
  let te=Math.floor(st.rewards.exp*diffMul.expMul),tg=Math.floor(st.rewards.gold*diffMul.goldMul);G.gold+=tg;
  const tc=G.tac;
  tc.waves.forEach(wave=>wave.forEach(e=>{te+=Math.floor((e.exp||0)*diffMul.expMul);tg+=Math.floor((e.gold||0)*diffMul.goldMul)}));
...
function showDef(){stopBGM();sfx('lose');G.mode='defeat';hideTacUI();$('defeat-screen').classList.add('on');addBattleLog('전투 패배...')}
