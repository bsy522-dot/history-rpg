...
## [AUTO] 2026-06-13 history-rpg — v16.0 기술트리24종+영웅도감12인+역사백과30항목+진형에디터6종Canvas+세력관계도Canvas+전투리플레이+일일임무4종+랭킹보드AI10인+전략가이드12팁+통계대시보드Canvas레이더+15퀴즈추가(120→135)+12업적추가(72→84)+SFX10종+키보드8종

### 1차: 벤치마킹 분석 (영걸전/문명/AOE 대비)

| 열위점 | 해결 방안 | 결과 |
|--------|----------|------|
| 기술 트리 없음 (문명 핵심) | 4시대 24기술 Canvas 트리 구현 | ✅ 해결 |
| 영웅 도감 부재 | 12인 영웅 상세 프로필+스킬+배경 | ✅ 해결 |
| 역사 백과사전 부족 | 30항목 12카테고리 검색형 백과 | ✅ 해결 |
| 진형 에디터 없음 | 6종 진형 Canvas 시각화+보너스 | ✅ 해결 |
| 세력 관계도 부재 | 8세력 Canvas 네트워크 맵 | ✅ 해결 |
| 전투 리플레이 없음 | 전투 로그 기록+재생 시스템 | ✅ 해결 |
| 일일 미션 없음 | 4종 일일 목표+연속 달성 스트릭 | ✅ 해결 |
| 랭킹 시스템 부재 | AI 10인 대전 순위표+등급 | ✅ 해결 |
| 전략 가이드 없음 | 12팁 확장형 공략 가이드 | ✅ 해결 |
| 통합 통계 부재 | 6축 레이더 Canvas+6메트릭 카드 | ✅ 해결 |

### 2차: 개발 내역

**v16_patch.js (850줄, ~55KB, 자기완결형 IIFE 패치 모듈)**

- **기술 트리**: 4시대(석기/청동기/철기/고대국가) 24기술 Canvas 520x580 인터랙티브 트리
  - 선행기술 연결선, 연구 가능/완료/불가 3상태 표시, 클릭 상세정보+연구 버튼
  - 금 소비 시스템 (50~1000), localStorage 영속, 통계 연동
- **영웅 도감**: 12인 (환웅/단군/비류왕/유리화/화랑/웅녀/해모수/위만/남려/계백/소서노/주몽)
  - 초상(이모지), 직업, 시대, HP/ATK/DEF/SPD 스탯, 고유스킬+설명, 배경스토리
- **역사 백과사전**: 30항목 12카테고리 (국가/유물/문화/역사/제도/기술/신앙/전투/경제/무기/이념)
  - 실시간 검색, 카테고리 탭 필터, 접기/펼치기, 열람 추적, 일일임무 연동
- **진형 에디터**: 6종 (학익진/어인진/장사진/방원진/추형진/원진) Canvas 400x300
  - 9유닛 배치 시각화, 진형별 공격/방어/이동 보너스, 원클릭 전환
- **세력 관계도**: 8세력 (고조선/부여/한나라/삼한/예/옥저/동예/진한) Canvas 520x340
  - 우호(녹)/적대(적)/중립(황) 관계선, 세력 크기 비례 원, 범례
- **전투 리플레이**: 20건 기록 저장, 승/패 표시, 전투 로그 아군/적군 색분리
- **일일 임무**: 전투3회/퀴즈5문제/기술연구/도감열람 4종, 연속 달성 스트릭, 자동 리셋
- **랭킹 보드**: AI 10인 (고주몽~세종대왕) 점수 순위, S/A/B/C/D 등급, 플레이어 실시간 순위
- **전략 가이드**: 12팁 (진형/자원/영웅조합/보스전/퀴즈/외교/장비/첩보/야영/카드/기술트리/일일미션)
- **통계 대시보드**: 6메트릭 카드 + Canvas 360x360 6축 레이더차트 (전투력/지식/외교/자원/업적/기술)
- **퀴즈 +15문** (120→135): 홍익인간/비파형동검/영고/소도/위만조선/한사군/다뉴세문경/고인돌/옥저/동예/팔조법금/천군/주몽/변한/미송리식토기
- **업적 +12개** (72→84): tech_first/tech_half/tech_all/hero_viewer/enc_reader/enc_master/form_changer/replay_first/daily_all/rank_top3/guide_reader/v16_explorer
- **SFX 10종**: tech_unlock/hero_open/enc_browse/form_place/fmap_hover/replay_play/daily_done/rank_view/guide_open/stats_view
- **키보드 8종**: Shift+T(기술트리)/H(영웅도감)/B(백과사전)/F(진형)/V(세력도)/D(일일임무)/K(랭킹)/G(공략) + Ctrl+Shift+S(통계)

**index.html**: v16.0 SEO 전면 갱신 (title/desc/keywords/OG/Twitter) + 신기능배지8종 + 84업적
**korean-rpg-v7.html**: v16.0 (title/desc/keywords + v16 타이틀 신기능)
**sw.js**: v15→v16 (krpg-v16 캐시, v16_patch.js PRECACHE+자동주입)
**manifest.json**: v16.0 설명+shortcuts 기술트리/영웅도감/역사백과/랭킹보드
...
|------|------|
| JS 문법 (node -c) | ✅ PASS |
| 괄호 밸런스 v16_patch.js | ✅ (=783 [=184 {=493 ALL BALANCED |
| 괄호 밸런스 index.html | ✅ (=142 [=5 {=91 ALL BALANCED |
| 괄호 밸런스 korean-rpg-v7.html | ✅ (=121 [=0 {=198 ALL BALANCED |
| div 밸런스 index.html | ✅ 19/19 |
| div 밸런스 korean-rpg-v7.html | ✅ 105/105 |
| CDN 사용 | ✅ 0건 (로컬 전용) |
| 개인정보 노출 | ✅ 0건 |

### 4차: 파일 변경 목록

- `js/v16_patch.js` — 신규 (850줄, IIFE 패치 모듈)
- `index.html` — v16.0 SEO 갱신
- `korean-rpg-v7.html` — v16.0 타이틀 갱신
- `sw.js` — v15→v16 캐시 갱신
- `manifest.json` — v16.0 설명+shortcuts 갱신
