...
## [AUTO] 2026-05-29 history-rpg — v13.0 장비강화+영웅특성트리+전투진형5종+영웅열전12인+전투통계대시보드+활쏘기미니게임+퀴즈15추가(90)+업적12추가(48)+SFX6종+키보드5종

### 1차: 벤치마킹 분석

**비교 대상:** 영걸전(삼국지 영걸전), 문명(Civilization), 에이지오브엠파이어(AOE)

| 항목 | 영걸전 | 문명 | AOE | history-rpg v12 | v13 해결 |
|------|--------|------|-----|-----------------|----------|
| 장비 강화/제련 | 무기+1~+10 | N/A | 기술업그레이드 | 기본 장비만 | ✅ +1~+10 강화 시스템 |
| 영웅 특성/스킬트리 | 전투마다 능력 성장 | 정책 트리 | 기술 트리 | 레벨업만 | ✅ 4갈래 특성트리 |
| 진형/편성 프리셋 | 진형 선택 가능 | 군사 편성 | 진형 선택 | 없음 | ✅ 5종 진형프리셋 |
| 캐릭터 상세 전기 | 인물열전 | 인물백과 | 문명백과 | 도감만 | ✅ 12인 영웅열전 |
| 전투 통계/분석 | 전적 기록 | 통계 대시보드 | 전적 표시 | 복기만 | ✅ Canvas 대시보드 |
| 미니게임 | 무쌍 이벤트 | 야만인 이벤트 | 사냥 | 없음 | ✅ 활쏘기 대회 |
| 퀴즈 범위 | N/A | 문명 퀴즈 | N/A | 75문항 | ✅ 90문항 |
| 업적 수 | 100+ | 200+ | 50+ | 36개 | ✅ 48개 |
| 사운드 효과 | 다양 | 다양 | 다양 | 기본 | ✅ +6종 추가 |
| 키보드 접근성 | 일부 | 전체 | 전체 | 제한적 | ✅ +5종 추가 |

### 2차: 개발 내역

**js/v13_patch.js: 신규 (913줄 ~58KB, 15섹션, 자기완결형 IIFE 패치 모듈)**

1. **장비 강화 시스템** (영걸전 스타일)
   - 12종 장비 (무기6+방어구4+장신구2) 강화 UI
   - +1~+10 강화 단계, 확률 시스템 (100%→8%)
   - 강화 비용 50G~1500G, 성공/실패 SFX+애니메이션
   - 강화 보너스: 공격+1~+20, 방어+0~+12, 속도+0~+4

2. **영웅 특성 트리**
   - 3종 병과별 4갈래 특성 (공격/방어/지원/특수)
   - 궁병: 정밀사격/관통사/난사/경갑수련/회피사격/원거리지원/연합사격/속사/연사
   - 보병: 강타수련/대지분쇄/무거운갑옷/철벽/불굴의의지/방패방어/돌파
   - 기마병: 돌격강화/철기돌파/기마회피/전장지휘/빠른기동/전격
   - 특성 포인트 소비, localStorage 영속

3. **전투 진형 프리셋 5종**
   - 화살촉진형: 궁병후방+보병전방 (궁병공+15%, 보병방+10%)
   - 방어진형: 보병밀집전방 (전체방+20%, 이동-1)
   - 돌격진형: 기마선두 (기마공+20%, 첫턴이동+2)
   - 유인진형: 소수유인+매복 (측면데미지+25%)
   - 혼합진형: 균형배치 (전체+5%)
   - Canvas 7x5 그리드 미니맵 시각화

4. **영웅 열전 12인**
   - 환웅/웅녀/단군왕검/풍백/우사/운사/해모수/금와왕/주몽/비류/위만/우거
   - 각 영웅 상세 전기 + 특수 능력 해설

5. **전투 통계 대시보드**
   - 4카드: 총전투/승률/총데미지/평균턴
   - MVP 영웅 표시 (최다 데미지)
   - Canvas 480x200 최근 10전투 바차트 (승/패 색상 구분)

6. **미니게임: 활쏘기 대회** (Canvas 480x320)
   - 바람 변수 (-3~+3), 화살 물리엔진 (중력+바람)
   - 과녁 5링 (10/30/50/80/100점)
   - 10발 발사, S/A/B/C/D 등급 판정
   - 최고점수 localStorage 저장

7. **퀴즈 +15문** (75→90)
   - 해모수/영고/범금8조/동예특산물/위만조선도읍/한사군/옥저골장제/변한/비파형동검/단군건국/소도/부여도둑질/360가지/무천/진한

8. **업적 +12개** (36→48)
   - enhance_first/enhance_5/enhance_max/talent_first/talent_10/formation_first/chronicle_first/stats_view/archery_first/archery_500/archery_master/quiz_90

9. **SFX 6종**
   - enhance_success/enhance_fail/talent_unlock/formation_set/arrow_shoot/arrow_hit

10. **키보드 단축키 +5종**
    - H=장비강화, T=특성트리, J=진형설정, N=영웅열전, O=전투통계

**파일 변경 목록:**
- js/v13_patch.js: 신규 (913줄)
- sw.js: v12→v13 (v13_patch.js PRECACHE+자동주입)
- index.html: v13.0 (SEO갱신+신기능배지8종+48업적)
- korean-rpg-v7.html: v13.0 (title/desc/keywords+v13스크립트태그)
- manifest.json: v13.0 설명+shortcuts 장비강화/영웅열전
...
| 중괄호 밸런스 | ✅ 409/409 BALANCED |
| 소괄호 밸런스 | ✅ 636/636 BALANCED |
| 대괄호 밸런스 | ✅ 125/125 BALANCED |
| HTML div 밸런스 (index.html) | ✅ 19/19 BALANCED |
| sw.js 문법 | ✅ PASS |
