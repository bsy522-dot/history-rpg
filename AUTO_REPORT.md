...
## [AUTO] 2026-05-17 history-rpg v11.0 — 역사도감35+일일퀘스트7종+콤보스킬6종+인물관계도15종+퀴즈55문+연대표+SFX4종+햅틱피드백+고대비모드+키보드단축키5종

### 1단계: 벤치마킹 분석 (영걸전/문명/AOE 대비)

| 항목 | 벤치마크 수준 | 이전 상태 | 개선 후 |
|------|-------------|----------|--------|
| 역사 도감 | 문명: 문명백과사전 | 없음 | ✅ 35항목 도감 (인물/사건/유물/지명/제도 5카테고리) |
| 일일 퀘스트 | MMO: 매일 갱신 임무 | 없음 | ✅ 7종 일일퀘스트 (날짜시드 로테이션) |
| 콤보 스킬 | 영걸전: 합체기 | 없음 | ✅ 6종 콤보 (2인 조합, 배율 1.8~2.5x) |
| 인물 관계도 | 삼국지: 인물 관계 | 없음 | ✅ 15종 관계 (SVG 인터랙티브 그래프) |
| 역사 퀴즈 | 교육+게임 융합 | 40문 | ✅ 55문 (+15문 고조선/부여/삼한) |
| 연대표 | 문명: 기술 트리 타임라인 | 없음 | ✅ 4챕터 연대표 (고조선~삼한) |
| SFX 다양성 | AoE: 상황별 효과음 | 8종 | ✅ 12종 (+콤보/도감/퀘스트/관계도 4종) |
| 햅틱 피드백 | 모바일게임: 진동 | 없음 | ✅ navigator.vibrate() 전투/업적/퀘스트 |
| 고대비 모드 | 웹표준: 접근성 | 없음 | ✅ CSS 고대비 테마 + 토글 |
| 키보드 단축키 | PC게임: 단축키 | 기본 | ✅ E/Q/C/R/H 5종 + 안내 패널 |
| 모션 감소 | 웹표준: prefers-reduced-motion | 없음 | ✅ CSS 미디어쿼리 대응 |

### 2단계: 개발팀 작업

#### js/v11_patch.js — 신규 (자체 완결 패치 모듈, ~41KB)
- **IIFE 구조**: v10_patch.js 패턴 준수, 전역 오염 없이 CSS/DOM/훅 주입
- **역사 도감 35항목 (ENC[])**: 
  - 인물 7: 단군, 환웅, 웅녀, 해모수, 주몽, 온조, 혁거세
  - 사건 7: 고조선건국, 기자조선, 위만조선, 한사군설치, 삼한형성, 부여건국, 가야건국
  - 유물 7: 비파형동검, 고인돌, 빗살무늬토기, 반달돌칼, 미송리식토기, 다뉴세문경, 철제농기구
  - 지명 7: 아사달, 왕검성, 위례성, 서라벌, 국내성, 금관가야, 마한목지국
  - 제도 7: 8조법금, 제천행사, 골품제, 화백회의, 소도, 순장, 읍락제
- **일일 퀘스트 7종 (DQ_DEFS[])**: 전투3회/퀴즈2회/콤보1회/도감5회/연승2회/힐20HP/아이템3회
  - 날짜시드 의사난수 로테이션 (매일 3개 활성)
  - localStorage 진행도 추적 (krpg_dq_date, krpg_dq_prog)
- **콤보 스킬 6종 (COMBOS[])**: 
  - 천지개벽(단군+환웅, 2.5x), 신궁합격(주몽+해모수, 2.2x), 삼한통일격(온조+혁거세, 2.0x)
  - 건국신화(단군+웅녀, 1.8x), 북방연합(해모수+주몽, 2.0x), 오국연합(온조+혁거세+주몽, 2.3x)
- **인물 관계도 15종 (RELS[])**: SVG 인터랙티브 그래프 렌더링
  - 부자/모자/사제/동맹/라이벌 관계 표현
  - 노드 클릭시 관계 하이라이트
  - 반응형 SVG viewBox
- **퀴즈 15문 추가 (40→55문)**: 고조선 영토, 단군신화, 비파형동검 용도, 제천행사 종류, 삼한 구성, 부여 4출도, 옥저 민며느리제, 동예 책화, 가야 철, 변한 위치, 고인돌 용도, 소도 의미, 위만 출신, 준왕 망명, 진국 위치
- **연대표 4챕터**: 고조선(BC2333~BC108), 열국시대(BC2C~AD1C), 삼한(BC1C~AD3C), 초기왕국(AD1C~AD4C)
- **SFX 4종 추가**: combo(콤보발동), encyclopedia(도감열람), quest_complete(퀘스트완료), relationship(관계도)
- **햅틱 피드백**: 전투 히트(50ms), 크리티컬(100ms+50ms), 업적달성(200ms), 퀘스트완료(150ms)
- **고대비 모드**: CSS 커스텀 프로퍼티 기반 고대비 테마, 토글 버튼
- **키보드 단축키 5종**: E(도감), Q(퀘스트), C(콤보), R(관계도), H(단축키도움말)
- **모션 감소**: prefers-reduced-motion 미디어쿼리 대응 (애니메이션 비활성화)
- **함수 훅**: onVictoryContinue(일일퀘스트 전투추적), answerQuiz(퀴즈추적)
- **메뉴 확장**: #menu-overlay에 도감/퀘스트/콤보/관계도/연대표 버튼 동적 삽입
- **window._v11 익스포트**: 런처에서 도감/퀘스트/콤보 접근용

#### sw.js — 수정 (서비스워커 캐시 + 스크립트 주입)
- 캐시명 `krpg-v10` → `krpg-v11` 업데이트
- 프리캐시 목록에 `./js/v11_patch.js` 추가
- **HTML 응답 인터셉트**: v10_patch.js에 이어 v11_patch.js도 `</body>` 앞에 자동 주입
- 네트워크 응답 + 캐시 응답 모두 주입 처리
- 중복 방지: `html.indexOf('v11_patch')<0` 가드

#### manifest.json — 수정
- description v11.0 업데이트: "도감35+일일퀘스트+콤보6종+관계도+55퀴즈+연대표+24업적+통계+다크모드+PWA"

#### index.html — v11.0 런처 업데이트
- 타이틀/메타 태그 v10.0 → v11.0
- OG/Twitter 메타 태그 description v11.0 업데이트
- 버전 문자열: "v10.0 — 24업적+통계+턴순서+40퀴즈+다크모드+PWA" → "v11.0 — 도감35+일일퀘스트+콤보6종+관계도+55퀴즈+연대표"
- 기능 배지 8종 v11 업데이트 (도감35/일일퀘스트/콤보6종/관계도/55퀴즈/연대표/다크모드/PWA)
- 게임 카드 v10→v11 라벨 + 신규 기능 태그
- 푸터 v10.0 → v11.0
...
| 검증 항목 | 결과 | 비고 |
|----------|------|------|
| 외부 CDN 사용 | ✅ 0건 | v11 신규/수정 파일에 외부 CDN 없음 (기존 Three.js 허용) |
| 개인정보 노출 | ✅ 0건 | 하드코딩된 개인정보 없음 |
| 파일 삭제 | ✅ 0건 | 기존 파일 보존, 신규 1 + 수정 3 |
| JS 구문 검증 | ✅ 통과 | v11_patch.js + sw.js node --check PASS |
| 핵심 파일 보존 | ✅ 완전 | korean-rpg-v7.html(32KB) 미수정 |
| 비침습 패턴 | ✅ 검증 | SW 주입으로 코어 파일 무변경 |
| localStorage 키 | ✅ 일관 | krpg_dq_date, krpg_dq_prog, krpg_enc 네이밍 |
| IIFE 격리 | ✅ 검증 | 전역 오염 window._v11만 |
| 접근성 | ✅ 추가 | 고대비 모드 + 모션 감소 대응 |
| 모바일 | ✅ 추가 | 햅틱 피드백 + 반응형 SVG |

### 파일 변경 목록
- `js/v11_patch.js` — **신규** (도감+퀘스트+콤보+관계도+퀴즈+연대표+SFX+햅틱+접근성 패치 모듈)
- `sw.js` — 캐시 v11 + 프리캐시 + HTML 스크립트 주입
- `manifest.json` — description v11.0 업데이트
- `index.html` — v11.0 런처 업데이트 (메타+배지+카드+푸터)
- `AUTO_REPORT.md` — v11.0 보고서 추가
