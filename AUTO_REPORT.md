...
## [AUTO] 2026-06-13 history-rpg — v16.0 왕실조정12관직+고대문자해독6종+민생경제시뮬+전장보고서Canvas+유적발굴8곳+역사시나리오6종+군사훈련소6종+세력연대기Canvas+15퀴즈추가(120→135)+12업적추가(72→84)+SFX10종+키보드8종

### 1차: 벤치마킹 분석 (영걸전/문명/AOE 대비)

| 열위점 | 해결 방안 | 결과 |
|--------|----------|------|
| 왕실 조정/관직 없음 (문명) | 12관직 임명 시스템 (좌보/우보/상가 등) + 12인 캐릭터 배치 | 해결 |
| 고대 문자 해독 없음 (교육) | Canvas 400x180 고대문자 렌더링 + 6종 해독 퍼즐 4지선다 | 해결 |
| 민생 경제 시뮬 없음 (문명) | 인구/세금/행복/식량/반란 5지표 + 6종 정책 액션 + 턴 경과 | 해결 |
| 전장 보고서/레이더 없음 (AOE) | Canvas 360x360 레이더 차트 6축 (전투력/방어력/사기/보급/전략/병력) | 해결 |
| 유적 발굴 없음 (문명) | 8곳 유적 (고인돌/비파형동검/세형동검 등) 깊이기반 발굴 | 해결 |
| 역사 분기 시나리오 없음 (영걸전) | 6종 역사적 이벤트 + 2갈래 선택지 + 결과 | 해결 |
| 군사 훈련소 없음 (AOE) | 6종 훈련 (검술/궁술/기마/진법/정신/지휘) Lv.1-10 + XP 시스템 | 해결 |
| 세력 연대기 없음 (문명) | Canvas 520x300 라인 그래프 3세력 12시기 | 해결 |
| 퀴즈 부족 (120) | +15문 추가 (120→135) 왕검성/천부경/청동기/고인돌 등 | 해결 |
| 업적 부족 (72) | +12개 추가 (72→84) 조정/해독/경제/보고서/발굴/시나리오/훈련/연대기 | 해결 |

### 2차: 개발 내역

**v16_patch.js: 신규 (901줄, 자기완결형 IIFE 패치 모듈)**
- 왕실 조정: 12관직 (좌보/우보/상가/대사자/대사/조의/패자/사자/봉례/집사/주부/교위) + 12인 캐릭터 + 임명/해임 + localStorage 저장
- 고대문자 해독: Canvas 400x180 고대문자 렌더 + 6종 퍼즐 (천부인/비파형/세형/고인돌/반달돌칼/미송리) + 4지선다
- 민생 경제: 인구/세금/행복/식량/반란 5지표 + 세율조절/경작확대/축제/구호/군비증강/교역 6액션 + 턴 시뮬
- 전장 보고서: Canvas 360x360 레이더 차트 6축 + 동적 데이터 기반
- 유적 발굴: 8곳 (비파형동검/세형동검/고인돌/반달돌칼/미송리식토기/천부인/청동거울/철제무기) + 깊이 진행형 발굴
- 역사 시나리오: 6종 (위만정변/한무제침공/왕검성공방/부여건국/삼한연맹/고구려건국) + 2선택지 + 결과
- 군사 훈련소: 6종 훈련 (검술/궁술/기마/진법/정신/지휘) + Lv.1-10 + XP 시스템 + localStorage
- 세력 연대기: Canvas 520x300 라인 그래프 + 고조선/부여/삼한 3세력 + 12시기 타임라인
- 퀴즈 +15문 (120→135): 왕검성/천부경/비파형동검지역/청동기시대/고인돌/위만출신/삼한/한사군/부여/마한 등
- 업적 +12개 (72→84): 조정관리/첫해독/해독마스터/경제시뮬/전장분석/첫발굴/발굴마스터/시나리오/훈련병/훈련교관/연대기/v16마스터
- SFX 10종: court_assign/court_dismiss/decode_correct/decode_wrong/economy_action/report_view/excavation_dig/scenario_choice/academy_train/chronicle_view
- 키보드 단축키 8종: Shift+Q(조정)/O(해독)/L(경제)/B(보고서)/X(발굴)/H(시나리오)/N(훈련소)/F(연대기)
- 메뉴 버튼 8종 자동 삽입 (사이드바 #menu-overlay에 동적 삽입)

**sw.js**: v15→v16 (krpg-v16, v16_patch.js PRECACHE+자동주입)
**index.html**: v16.0 SEO 전면 갱신 (title/desc/keywords/OG/Twitter/JSON-LD) + 신기능배지8종 + 84업적 + 135퀴즈
**korean-rpg-v7.html**: v16.0 (title/desc/keywords 갱신)
**manifest.json**: v16.0 설명 + shortcuts 왕실조정/유적발굴/역사시나리오/군사훈련소

### 3차: 품질검증 결과

| 항목 | 결과 |
|------|------|
| JS 문법 (v16_patch.js) | PASS |
| JS 문법 (전체 14파일) | 14/14 PASS |
| 괄호 밸런스 () | 615/615 OK |
| 괄호 밸런스 {} | 388/388 OK |
| 괄호 밸런스 [] | 103/103 OK |
...
| 파일 | 변경 | 줄수 |
|------|------|------|
| js/v16_patch.js | 신규 | 901줄 |
| index.html | 수정 | 184줄 |
| korean-rpg-v7.html | 수정 | 406줄 |
| sw.js | 수정 | 54줄 |
| manifest.json | 재작성 | 45줄 |
