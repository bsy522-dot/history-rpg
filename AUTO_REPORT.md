...
## [AUTO] 2026-06-23 history-rpg — v18.0 전략자문회의Canvas+역사연표20사건Canvas+군량미관리소Canvas+정찰망구축Canvas12x10+전설무기제련소12종+역사명장면극장12편+전쟁보고서Canvas+병영훈련소3종+퀴즈+15(150→165)+업적+12(96→108)+SFX12종+키보드8종

### 1차: 벤치마킹 분석 (영걸전/문명/AOE 대비)

| 열위점 | 해결 방안 | 결과 |
|--------|----------|------|
| 전략 자문/고문관 없음 (문명) | 4명 자문관 + 국력 레이더 Canvas | v18_patch.js |
| 역사 연표 시각화 없음 | 20대 사건 타임라인 Canvas 클릭 탐험 | v18_patch.js |
| 보급/군량 관리 없음 (AOE) | 군량/무기/약재/마 4종 + 바차트 Canvas | v18_patch.js |
| 정찰/스카우팅 없음 (문명) | 12x10 정찰 Canvas, 첩보원 배치+발견 | v18_patch.js |
| 전설 장비 제작 없음 (영걸전) | 12종 전설무기 재료수집→제작 시스템 | v18_patch.js |
| 역사 명장면 연출 없음 | 12대 명장면 이모지 씬+나레이션+역사적 사실 | v18_patch.js |
| 전쟁 통계 보고서 없음 | Canvas 6메트릭 바차트 + PNG다운/클립보드 | v18_patch.js |
| 유닛 훈련 미니게임 없음 (AOE) | 검술/궁술/기마 3종 Canvas 미니게임 | v18_patch.js |
| 퀴즈 부족 (165 목표) | +15문 (150→165) 고조선~삼한 | v18_patch.js |
| 업적 부족 (108 목표) | +12개 (96→108) | v18_patch.js |

### 2차: 개발 내역

**v18_patch.js**: 신규 (1158줄 ~58KB, 자기완결형 IIFE 패치 모듈)

1. **전략 자문회의** (Strategic Council Canvas 400x400)
   - 을지무공(군사)/해모수(문화)/위만(경제)/유화부인(외교) 4명 자문관
   - 국력 4축 Radar Canvas (군사력/문화력/경제력/외교력)
   - 현재 상태 기반 동적 조언 생성
   - 종합 국력 S~D등급 평가

2. **역사 연표 탐험기** (Timeline Canvas 540x600)
   - BC 2333 단군건국 ~ AD 53 태조왕 즉위까지 20대 사건
   - 클릭 탐험 + 사건별 상세 설명
   - 고조선/위만조선/부여/삼한/열국 5시대 색상 구분
   - 탐험 진행률 추적 + localStorage 저장

3. **군량미 관리소** (Supply Canvas 480x240)
   - 군량미/무기/약재/군마 4종 보급품
   - 수집/긴급조달 버튼 + 자동 생산율
   - Canvas 바차트 보급 현황 시각화
   - 최대치 도달 시 업적 해금

4. **정찰망 구축기** (Scout Canvas 480x400)
   - 12x10 = 120셀 정찰 지도
   - 7종 지형 (평원/산악/강/마을/적진/유적/첩보원)
   - 클릭 탐사 + 첩보원 배치(주변 자동 발견)
   - 대규모 정찰 5셀 동시 탐사

5. **전설무기 제련소**
   - SSR 2종 (단군검/환웅바람활) + SR 3종 + R 4종 + N 3종 = 12종
   - 12종 재료 (철/금/영혼석/깃털/수정/돌/뼈/어금니/목재/가죽/비단/옥)
   - 재료 채집 + 제련 시스템
   - 전설무기별 고유 보너스 효과

6. **역사 명장면 극장**
   - 12대 명장면 (천지창조~거석의비밀)
   - 이모지 씬 연출 + 나레이션 + 역사적 사실
   - 관람 기록 추적 + 업적

7. **전쟁 보고서 생성기** (Report Canvas 600x380)
   - 6메트릭 바차트 (전투승리/퀴즈정답/업적/무기/보급/연표)
   - 종합 점수 S~D등급 산출
   - PNG 다운로드 + 클립보드 복사

8. **병영 훈련소** (Training Canvas 400x300)
   - 검술/궁술/기마 3종 미니게임
   - 10초 타겟 클릭 → 점수 → S~D등급
   - 훈련 레벨 누적 + 능력치 강화

**퀴즈 +15문** (150→165): 8조법금/위만경로/비파형동검/영고/한사군/변한/왕검성/단군건국/옥저/동예/세력범위/부여관직/천군/도둑질/철기

**업적 +12개** (96→108): council_first/council_all/timeline_explorer/timeline_10/supply_manager/supply_full/scout_first/scout_master/forge_first/forge_3/theater_first/training_first

**SFX 12종**: council_open/council_advice/timeline_scroll/timeline_click/supply_collect/supply_produce/scout_deploy/scout_discover/forge_craft/forge_upgrade/theater_open/theater_scene

**키보드 8종**: Shift+F(자문)/G(연표)/H(보급)/I(정찰)/K(제련소)/N(극장)/O(보고서)/R(훈련소)

**하단 스크롤 네비바 8종 추가** (v17 네비바에 확장)
...
|------|------|
| JS 문법 (node -c) | PASS |
| 괄호 밸런스 | ALL BALANCED (940/437/130) |
| div 태그 밸런스 | index 19/19, v7 105/105, v18 29/29 |
| CDN 사용 | 0건 |
| 개인정보 노출 | 0건 |
| 파일 크기 | 1158줄 58KB |
| 모바일 반응형 | @media 480px 대응 |
| 다크모드 | CSS 변수 연동 |
| localStorage | 6종 데이터 영속 |
| Service Worker | v17→v18 캐시 갱신, v18_patch.js PRECACHE |
...
## [AUTO] 2026-06-19 history-rpg v17.0

### 1차: 벤치마킹 (영걸전 / 문명 / AOE 대비)

| 열위점 | 경쟁 앱 | v17 해결 |
|--------|---------|----------|
| 전쟁안개(Fog of War) 없음 | 문명/AOE | ✅ 16x12 Canvas 탐사맵, 클릭/정찰/전체탐사 |
| 왕조 계보도 없음 | 영걸전/삼국지 | ✅ 환인~온조 12인 Canvas 계보도 |
| 군대 편성/병종 다양성 부족 | AOE/영걸전 | ✅ 10병종 6슬롯 편대 편성기 |
| 동맹/조약 시스템 없음 | 문명 | ✅ 5국 동맹체결 (선물/교역/동맹/위협) |
| 승리 조건 추적 없음 | 문명 다중승리 | ✅ 군사/문화/경제/외교 4종 Canvas 추적기 |
| 역사 랜덤 이벤트 부족 | 문명/영걸전 | ✅ 12종 이벤트 카드 (재해/축복/외교/자원) |
| 군대 사기(모럴) 시스템 없음 | 영걸전/AOE | ✅ 7요인 사기 게이지 Canvas + 4종 부스트 |
| 전장 지형 편집 없음 | AOE/문명 | ✅ 14x10 8종 지형 Canvas 시뮬레이터 |
| 퀴즈 135문 (부족) | 교육 앱 | ✅ +15문 → 150문 |
| 업적 84종 (부족) | 게임 표준 | ✅ +12종 → 96종 |

### 2차: 개발 상세

**v17_patch.js** (888줄, ~55KB, 자기완결형 IIFE 패치 모듈)

1. **전쟁안개 탐사 Canvas**: 16x12 Fog of War 맵, 8종 타일(평원/숲/산/강/마을/요새/유적/동굴), 클릭 반경2 탐사, 정찰 반경3, 전체탐사 200G, 탐사율 추적
2. **왕조 계보도 Canvas**: 환인→환웅→단군→해부루→금와, 해모수+유화→주몽→비류/온조/유리 12노드 계보도, 혈통선 점선+컬러코딩
3. **편대 편성기**: 10병종(보병/궁병/기병/무당/장군/척후병/방패병/공성병/승려/첩자) 6슬롯 드래그배치, 합산 공격/방어 표시
4. **동맹 체결 시스템**: 5국(부여/한/예맥/옥저/삼한) 외교 4액션(선물100G/교역/동맹/위협), 우호도 -100~+100, 상태표시
5. **승리 조건 추적 Canvas**: 군사(30승)/문화(200점)/경제(5000G)/외교(5개국동맹) 4종 프로그레스바 Canvas 시각화
6. **역사 이벤트 카드 12종**: 대홍수/풍년/별똥별/역병/사절단/철광맥/반란/무역선/일식/영웅탄생/가뭄/고대두루마리
7. **전투 사기 시스템**: 7요인(통솔/보급/승리/날씨/휴식/신앙/규율) 사기 게이지 Canvas, S~D등급, 격려/보급/제천/휴식 4종 부스트
8. **전장 지형 시뮬레이터 Canvas**: 14x10 8종 지형(초원/숲/언덕/강/사막/요새/늪/도로) 클릭 배치, 방어보너스/이동비용 표시

**추가 콘텐츠**:
- 퀴즈 +15문 (135→150): 고조선건국/환웅신하/홍익인간/8조법/부여관직/동예특산/소도/변한/아사달/제정분리/고인돌/비파형동검/옥저장례/민무늬토기/사기
- 업적 +12개 (84→96): 탐험가/지도완성자/계보학자/편대편성관/동맹의달인/만국평화/정복자/승리자/역사의증인/사기충천/지형설계가/v17탐험가
- SFX 12종 Web Audio API: fog_reveal/dynasty_open/squad_assign/diplomacy/victory_progress/event_trigger/morale_change/terrain_place/morale_boost/alliance_treaty/event_card/achieve_v17
- 키보드 8종: Shift+A(전쟁안개)/B(계보도)/C(편대)/D(동맹)/E(승리조건)/J(이벤트)/M(사기)/P(지형)
- 하단 가로 스크롤 네비게이션 바 (v17 8기능 터치 접근성)

**파일 변경**:
- `js/v17_patch.js` — 신규 (888줄)
- `korean-rpg-v7.html` — v17 SEO/메타/타이틀/피처 + v17 스크립트태그
- `index.html` — v17.0 SEO/피처배지/카드/푸터 전면 갱신
- `sw.js` — v16→v17 (krpg-v17 캐시, v17_patch.js PRECACHE + 자동주입)
- `manifest.json` — v17.0 설명 + shortcuts 6종
...
| 괄호 균형 {} | ✅ 353/353 |
| 괄호 균형 [] | ✅ 111/111 |
| div 균형 (index.html) | ✅ 19/19 |
| div 균형 (korean-rpg-v7.html) | ✅ 105/105 |
| CDN 외부 참조 | ✅ 0건 |
| 개인정보 노출 | ✅ 0건 |
| SW 캐시 갱신 | ✅ krpg-v17 |
