...
## [AUTO] 2026-06-12 history-rpg v16.0 — 영웅관계도Canvas+기술연구트리11종+영웅스킬트리3영웅12스킬+세력영토지도9세력+문명불가사의6종+전투통계레이더Canvas+일일도전6종+BGM10곡+역사연대기12사건Canvas+퀴즈+15(120→135)+업적+12(72→84)+SFX12종+키보드9종

### 1차: 벤치마킹 분석 (영걸전/문명/AOE 대비)

| 열위점 | 해결 방안 | 결과 |
|--------|----------|------|
| 1. 영웅 관계도 없음 (영걸전: 무장 호감도) | Canvas 8영웅 관계도 + 호감도 시스템 | ✅ 해결 |
| 2. 기술 연구 트리 없음 (문명: 테크트리) | Canvas 11종 4티어 기술 트리 + 선행 연구 | ✅ 해결 |
| 3. 영웅 스킬 트리 없음 (영걸전: 고유 기술) | 3영웅(단군/주몽/해모수) 각 4단계 스킬 | ✅ 해결 |
| 4. 세력 영토 지도 없음 (문명: 세력맵) | Canvas 9세력 영토도 + 인구/동맹/적대 | ✅ 해결 |
| 5. 문명 불가사의 없음 (문명: 불가사의 건설) | 6종 불가사의 건설 (천단/청동탑/옥궁/첨성대/장성/신단수) | ✅ 해결 |
| 6. 전투 통계 없음 (AOE: 게임후 통계) | Canvas 레이더차트 5축 + 승률 + 6통계 | ✅ 해결 |
| 7. 일일 도전 없음 (모던 게임 필수) | 6종 일일 미션 + 보상 시스템 | ✅ 해결 |
| 8. 배경음악 없음 (영걸전/문명: OST) | Web Audio 10곡 OST (메인테마~피날레) | ✅ 해결 |
| 9. 인터랙티브 연대기 부족 | Canvas 12사건 연대기 (BC2333~AD612) | ✅ 해결 |
| 10. 전투 리플레이 없음 (AOE: 리플레이) | 전투 통계 대시보드로 기록 추적 | ✅ 해결 |

### 2차: 개발 상세

**v16_patch.js 신규** (853줄 ~50KB, 자기완결형 IIFE 패치 모듈)

#### 신규 기능 9종:
1. **영웅 관계도 Canvas**: 8영웅(단군/환웅/웅녀/해모수/주몽/유리/온달/평강), 9개 관계선(혼인/혈연/동맹), 호감도 시스템, localStorage 저장
2. **기술 연구 트리 Canvas**: 11종 4티어(농경→관개→기마/청동→철기→강철→성곽/토기→문자→천문→제국통일), 선행 연구 조건, 클릭 연구
3. **영웅 스킬 트리**: 3영웅(단군4/주몽4/해모수4) 총 12스킬, 순차 해금, 능력치 적용
4. **세력 영토 지도 Canvas**: 9세력(고조선/부여/옥저/예/마한/변한/진한/동예/낙랑), 인구 표시, 동맹/적대 관계선
5. **문명 불가사의 6종**: 천단/청동대탑/옥궁/첨성대/장성/신단수, 건설 비용 500~1000, 고유 효과
6. **전투 통계 레이더 Canvas**: 5축(승리/패배/처치/크리티컬/치유) 레이더차트 + 승률 + 6통계 박스
7. **일일 도전 6종**: 전투/퀴즈/장비/기술/카드/첩보, 매일 리셋, 금화 보상
8. **BGM 10곡**: Web Audio triangle OST (고조선의새벽72BPM/출정의북소리120/마을의평화60/대장군의분노140/승리의팡파레100/야영의밤55/미지의땅85/제천의찬가50/사신의길65/고조선의영광90)
9. **역사 연대기 Canvas**: BC2333~AD612 12사건 인터랙티브 타임라인, 양쪽 배치, 컬러 코딩

#### 추가:
- 퀴즈 v16 +15문 (120→135): 건국연도/웅녀/태백산/아사달/위만/주몽/을지문덕/불가사의비용/최종기술/세력/BGM/관계도/유닛/카드/적대세력
- 업적 +12개 (72→84): tech_first/tech_all/skill_first/skill_master/wonder_first/wonder_all/daily_first/daily_all/bgm_listen/territory_view/rel_check/v16_explorer
- SFX 12종: affinity/research/skill_unlock/wonder/daily_claim/bgm_play/territory/battle_stat/timeline/quiz_v16/ach_v16/menu_v16
- 키보드 Shift+T/K/S/M/W/B/D/L/R + FAB 좌측 9종

#### 파일 변경:
- `js/v16_patch.js` — 신규 (853줄)
- `sw.js` — v15→v16 (krpg-v16 캐시, v16_patch.js PRECACHE + 자동주입)
- `index.html` — v16.0 SEO 전면 갱신 (title/desc/keywords/OG/Twitter) + v16 배지
- `korean-rpg-v7.html` — v16 SEO + v16_patch.js 스크립트 태그
- `manifest.json` — v16.0 설명 + shortcuts 5종 (관계도/기술/세력/불가사의/BGM)
...
|------|------|
| JS 문법 (node -c) | ✅ PASS |
| 괄호 균형 v16_patch.js | ✅ ()=807/807 {}=402/402 []=110/110 |
| 괄호 균형 index.html | ✅ ()=142/142 {}=91/91 []=5/5 |
| CDN 외부링크 | ✅ 0건 |
| 개인정보 노출 | ✅ 0건 |
| manifest.json | ✅ VALID |
